...
use substrate_test_runtime_client::{BlockBuilderExt, runtime::Header, ClientExt};
		...
		for block_num in 0..count {
			...
			let built_hashes = self.peer(0).generate_blocks(1, BlockOrigin::File, |mut builder| {
				...
				if include_mmr_digest {
					...
					add_mmr_digest2(&mut builder, mmr_root);
				}

				if block_num % session_length == 0 {
					add_auth_change_digest2(&mut builder, validator_set.clone());
				}
...
fn add_mmr_digest2(builder: &mut impl BlockBuilderExt, mmr_hash: MmrRootHash) {
...
fn add_auth_change_digest2(builder: &mut impl BlockBuilderExt, new_auth_set: BeefyValidatorSet) {
	let _ = builder.push_deposit_log_digest_item(
		DigestItem::Consensus(
			BEEFY_ENGINE_ID,
			ConsensusLog::<AuthorityId>::AuthoritiesChange(new_auth_set).encode(),
