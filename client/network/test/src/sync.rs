...
fn sync_justifications() {
	...
	let headerof10 = net.peer(1).client().header(&BlockId::Number(10)).unwrap().unwrap();
	let headerof15 = net.peer(1).client().header(&BlockId::Number(15)).unwrap().unwrap();
	let headerof20 = net.peer(1).client().header(&BlockId::Number(20)).unwrap().unwrap();

	// peer 1 should get the justifications from the network
	net.peer(1).request_justification(&headerof10.hash(), 10);
	net.peer(1).request_justification(&headerof15.hash(), 15);
	net.peer(1).request_justification(&headerof20.hash(), 20);

	block_on(futures::future::poll_fn::<(), _>(|cx| {
		net.poll(cx);

		for hash in [&headerof10,&headerof15,&headerof20].map(Header::hash) {
